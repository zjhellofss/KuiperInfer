 ...
 * @return 创建后的张量
 */
std::shared_ptr<Tensor<float>> TensorCreate(std::vector<uint> shapes);


std::tuple<std::shared_ptr<Tensor<float>>, std::shared_ptr<Tensor<float>>>TensorBroadcast(const sftensor& tensor1,
                                               const sftensor& tensor2);
}  // namespace kuiper_infer
