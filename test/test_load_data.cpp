...
TEST(test_load, load_csv_data) {
  ...
  const arma::mat &data = CSVDataLoader::LoadData("./tmp/data1.csv");
...
TEST(test_load, load_csv_arange) {
  using namespace kuiper_infer;
  const arma::mat &data = CSVDataLoader::LoadData("./tmp/data2.csv");
  ASSERT_NE(data.empty(), true);
  ASSERT_EQ(data.n_rows, 3);
  ASSERT_EQ(data.n_cols, 4);
...
TEST(test_load, load_csv_missing_data1) {
  using namespace kuiper_infer;
  const arma::mat &data = CSVDataLoader::LoadData("./tmp/data4.csv");
  ASSERT_NE(data.empty(), true);
  ASSERT_EQ(data.n_rows, 3);
...
TEST(test_load, load_csv_missing_data2) {
  using namespace kuiper_infer;
  const arma::mat &data = CSVDataLoader::LoadData("./tmp/data3.csv");
...
TEST(test_load, load_minus_data) {
  using namespace kuiper_infer;
  const arma::mat &data = CSVDataLoader::LoadData("./tmp/data6.csv", ',');
